...
# =====================================================
# 09_conformal.R  —  Section 4: Weighted / nonexchangeable conformal (Barber et al. 2023)
# -----------------------------------------------------
# Second adaptive method, to compare against ACI. Where ACI ADAPTS THE
# THRESHOLD via feedback, weighted conformal RE-WEIGHTS THE CALIBRATION
# SAMPLE: recent calibration loans count more, stale ones less, so the
# (weighted) quantile reflects the recent world under drift.
#   - growing calibration pool: 2013-H2 calib + all OBSERVED prior test quarters
#   - exponential recency weight exp(-lambda * age_in_quarters)
#   - per-class (Mondrian) structure, like frozen Mondrian and ACI
# Citation: Barber, Candes, Ramdas & Tibshirani (2023), "Conformal
# prediction beyond exchangeability". Prereq: Sections 1-3 have run.
# =====================================================

stopifnot(exists("mondrian"), exists("aci_res"), exists("score_set"),
          exists("models_spec"), exists("quarters"))

lambdas <- c(0.2, 0.5, 1.0)         # recency decay rates (slow / med / fast)

# Weighted (1-alpha) quantile: smallest score whose cumulative normalised
# weight reaches 1-alpha.
weighted_quantile <- function(scores, weights, alpha) {
  o <- order(scores); s <- scores[o]; w <- weights[o]
  cw <- cumsum(w) / sum(w)
  idx <- which(cw >= (1 - alpha))[1]
  if (is.na(idx)) idx <- length(s)
  s[idx]
}

# For a model: assemble per-quarter (p, y) once (reused across lambdas)
gather_quarters <- function(spec) {
  qs <- lapply(quarters, function(qt) {
    s <- score_set(spec, qt); ok <- is.finite(s$p) & is.finite(s$y)
    list(p = s$p[ok], y = s$y[ok])
  }); names(qs) <- quarters
  cal <- score_set(spec, "calib"); okc <- is.finite(cal$p) & is.finite(cal$y)
  list(cal = list(p = cal$p[okc], y = cal$y[okc]), q = qs)
}

run_weighted <- function(dat, lambda, alpha = 0.10) {
  res <- vector("list", length(quarters))
  for (qi in seq_along(quarters)) {
    qt <- quarters[qi]
    
    # --- build the weighted, per-class calibration pool for THIS quarter ---
    # age 0 = most recent available block; calib block is the oldest.
    # Blocks: calib (oldest), then test quarters 1..(qi-1) in order.
    pool_p <- dat$cal$p; pool_y <- dat$cal$y
    pool_age <- rep(qi, length(dat$cal$p))          # calib is qi quarters "back"
    if (qi >= 2) for (j in seq_len(qi - 1)) {
      pj <- dat$q[[quarters[j]]]
      pool_p <- c(pool_p, pj$p); pool_y <- c(pool_y, pj$y)
      pool_age <- c(pool_age, rep(qi - j, length(pj$p)))  # closer quarter = smaller age
    }
    w <- exp(-lambda * pool_age)
    
    # per-class weighted thresholds
    g <- pool_y == 0; b <- pool_y == 1
    thr_good <- weighted_quantile(pool_p[g],      w[g], alpha)   # score_good = p
    thr_bad  <- weighted_quantile(1 - pool_p[b],  w[b], alpha)   # score_bad  = 1-p
    
    # --- apply to current quarter ---
    cq <- dat$q[[qt]]; p <- cq$p; y <- cq$y
    good_in <- p <= thr_good; bad_in <- (1 - p) <= thr_bad
    covered <- ifelse(y == 1, bad_in, good_in)
    res[[qi]] <- data.table(quarter = qt,
                            coverage = round(mean(covered), 4),
                            cov_bad  = round(mean(covered[y == 1]), 4),
                            cov_good = round(mean(covered[y == 0]), 4),
                            pct_uncert = round(mean((good_in + bad_in) == 2) * 100, 1))
  }
  rbindlist(res)
}

focal <- "lgbm_full"
dat   <- gather_quarters(models_spec[[focal]])
weighted_res <- rbindlist(lapply(lambdas, function(L) {
  r <- run_weighted(dat, L); r[, lambda := L]; r
}))

cat("\n=== WEIGHTED conformal: bad-loan coverage by quarter, focal =", focal,
    "(target 0.90) ===\n")
print(dcast(weighted_res, quarter ~ lambda, value.var = "cov_bad"))

# --- Head-to-head: the three drift responses on bad-loan coverage ---
cat("\n=== Bad-loan coverage: frozen Mondrian vs ACI vs Weighted (lambda=0.5) ===\n")
cmp3 <- Reduce(function(a, b) merge(a, b, by = "quarter"), list(
  mondrian[model == focal, .(quarter, frozen_mondrian = cov_bad)],
  aci_res[gamma == 0.02, .(quarter, ACI = cov_bad)],
  weighted_res[lambda == 0.5, .(quarter, weighted = cov_bad)]
))
print(cmp3[order(quarter)])